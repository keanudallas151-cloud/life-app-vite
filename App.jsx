...
export default function LifeApp() {
  ...
  // Register form (kept — still used by register screen)
  ...
  const doRegister = async () => {
    ...
    if (!rUsername.trim()) err.username = "Username is required.";
      ...
      const caps = (rUsername.match(/[A-Z]/g) || []).length;
      if (caps > 1) err.username = "Only one capital letter allowed.";
    }
  ...
  if (screen === "signin") return (
    <div style={{ minHeight: "100vh", background: C.skin, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "Georgia,serif", padding: "40px 28px" }}>
      ...
      <h2 style={{ fontSize: 26, fontWeight: 700, margin: "0 0 8px", color: C.ink, fontFamily: "Georgia,serif" }}>
        Sign in to Life.
      </h2>
      <p style={{ margin: "0 0 36px", fontSize: 14, color: C.muted, fontStyle: "italic" }}>
      ...
      <div style={{ width: "100%", maxWidth: 300, display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 14, width: "100%", background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 14, padding: "16px 20px", cursor: authLoading ? "default" : "pointer", fontFamily: "Georgia,serif", boxSizing: "border-box", opacity: authLoading ? 0.7 : 1, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}
        >
          <img src="/google_login.png" alt="Google" style={{ width: 26, height: 26, objectFit: "contain", flexShrink: 0 }} />
        ...
        {/* Apple — placeholder */}
        <button
          onClick={() => { play("tap"); setSiSocialErr("Apple login is coming soon."); }}
          style={{ display: "flex", alignItems: "center", gap: 14, width: "100%", background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 14, padding: "16px 20px", cursor: "pointer", fontFamily: "Georgia,serif", boxSizing: "border-box", opacity: 0.5, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}
        >
          <img src="/apple_login.png" alt="Apple" style={{ width: 26, height: 26, objectFit: "contain", flexShrink: 0 }} />
          <span style={{ fontSize: 15, fontWeight: 600, color: C.ink }}>Continue with Apple</span>
        </button>

        {/* Twitter — placeholder */}
        <button
          onClick={() => { play("tap"); setSiSocialErr("Twitter login is coming soon."); }}
          style={{ display: "flex", alignItems: "center", gap: 14, width: "100%", background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 14, padding: "16px 20px", cursor: "pointer", fontFamily: "Georgia,serif", boxSizing: "border-box", opacity: 0.5, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}
        >
          <img src="/twitter_login.png" alt="Twitter" style={{ width: 26, height: 26, objectFit: "contain", flexShrink: 0 }} />
          <span style={{ fontSize: 15, fontWeight: 600, color: C.ink }}>Continue with Twitter</span>
        </button>

        {/* Phone — placeholder */}
        <button
          onClick={() => { play("tap"); setSiSocialErr("Phone login is coming soon."); }}
          style={{ display: "flex", alignItems: "center", gap: 14, width: "100%", background: C.white, border: `1.5px solid ${C.border}`, borderRadius: 14, padding: "16px 20px", cursor: "pointer", fontFamily: "Georgia,serif", boxSizing: "border-box", opacity: 0.5, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}
        >
          <img src="/phone_login.png" alt="Phone" style={{ width: 26, height: 26, objectFit: "contain", flexShrink: 0 }} />
          <span style={{ fontSize: 15, fontWeight: 600, color: C.ink }}>Continue with Phone</span>
        </button>

        {siSocialErr && (
          <p style={{ margin: "4px 0 0", fontSize: 12, color: C.red, textAlign: "center", fontStyle: "italic", lineHeight: 1.6 }}>
            {siSocialErr}
          </p>
        )}

        <button
          onClick={() => { play("tap"); setScreen("landing"); setSiSocialErr(""); }}
          style={{ background: "none", border: "none", color: C.muted, fontSize: 13, cursor: "pointer", fontFamily: "Georgia,serif", fontStyle: "italic", marginTop: 4 }}
        >
          ← Back
        </button>
        ...
        <p style={{ textAlign: "center", color: C.muted, fontSize: 13, margin: 0 }}>
          No account?{" "}
          <button
            onClick={() => { play("tap"); setScreen("register"); setSiSocialErr(""); }}
  ...
  if (screen === "register") return (
    <div style={{ minHeight: "100vh", background: C.skin, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", fontFamily: "Georgia,serif", padding: "40px 28px" }}>
      <div style={{ width: 70, height: 70, borderRadius: "20%", background: `linear-gradient(145deg,${C.green},#2d6e42)`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20, boxShadow: "0 4px 16px rgba(31,150,61,0.5)" }}>
        <span style={{ color: "#fff", fontSize: 28, fontWeight: 800 }}>l.</span>
      </div>
      ...
      <div style={{ width: "100%", maxWidth: 340, display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div>
          <Field label="Username" value={rUsername} onChange={setRUsername} error={rErr.username} placeholder="e.g. DavidCaven01" />
          <p style={{ margin: "4px 0 0", fontSize: 9, color: C.muted, fontStyle: "italic" }}>6–15 chars. One capital max. No special characters.</p>
        </div>
        <Field label="Date of Birth" value={rDob}
          onChange={(v) => {
            let d = v.replace(/\D/g, "").slice(0, 6);
            let f = "";
            if (d.length >= 2) { f += d.slice(0, 2); } else { f += d; }
            if (d.length >= 3) { f += "/" + d.slice(2, 4); }
            if (d.length >= 5) { f += "/" + d.slice(4, 6); }
            setRDob(f);
          }}
          error={rErr.dob} placeholder="dd/mm/yy" />
        <Field label="Password" type="password" value={rPass} onChange={setRPass} error={rErr.pass} placeholder="Min 6 chars + special character" />
        ...
        <button onClick={doRegister} disabled={authLoading}
          style={{ background: C.green, border: "none", borderRadius: 12, padding: "17px", color: "#fff", fontSize: 16, fontWeight: 700, cursor: authLoading ? "default" : "pointer", fontFamily: "Georgia,serif", marginTop: 4, opacity: authLoading ? 0.7 : 1 }}>
        ...
        <p style={{ fontSize: 11, color: C.muted, textAlign: "center", lineHeight: 1.7, margin: "4px 0 0", fontStyle: "italic" }}>
          By creating an account you agree to our{" "}
          <a href="/terms.html" target="_blank" rel="noopener noreferrer" style={{ color: C.green, fontWeight: 600, textDecoration: "none" }}>Terms of Service</a>
          {" "}and{" "}
          <a href="/privacy.html" target="_blank" rel="noopener noreferrer" style={{ color: C.green, fontWeight: 600, textDecoration: "none" }}>Privacy Policy</a>.
        </p>
      </div>
    </div>
  );
